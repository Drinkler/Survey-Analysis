...
opensourcer <- dbGetQuery(con,
                          "SELECT OpenSourcer, COUNT(OpenSourcer) AS Amount
                          FROM survey
...
ggplot(opensourcer_df, aes(x = reorder(OpenSourcer, Amount), y = Amount)) +
  geom_bar(stat = "identity", color = "#5AA2F9", fill = "#8BBEFB", width = 0.8) + 
  coord_flip() + 
  theme_minimal() +
  ...
  geom_text(aes(label = (Amount/sum(Amount)) %>% scales::percent(decimal.mark = ",")), hjust = -0.1)
